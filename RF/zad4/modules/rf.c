...
void address_write(unsigned char ucAddr[], unsigned char ucLength) {
	unsigned char ucCtr;
	set_CS(0);
	SPI_MasterTransceiveByte(&rf_spi_master,TX_ADDR);
	for (ucCtr=0;ucCtr<ucLength;ucCtr++) SPI_MasterTransceiveByte(&rf_spi_master,ucAddr[ucCtr]);
	set_CS(1);
}
